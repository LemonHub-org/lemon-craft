...
/// `RUST_LOG="lemoncraft_voxygen=trace"`
///
    ...
    <W2 as MakeWriter<'static>>::Writer: 'static + Send + Sync,
{
    ...
    // this crate would be lemoncraft_voxygen=debug.

    let mut filter = EnvFilter::default().add_directive(LevelFilter::INFO.into());

    let default_directives = [
        "dot_vox::parser=warn",
        "lemoncraft_common::trade=info",
        "lemoncraft_world::sim=info",
        "lemoncraft_world::civ=info",
        "lemoncraft_world::site::economy=info",
        "lemoncraft_server::events::entity_manipulation=info",
        "hyper=info",
        ...
        "lemoncraft_network_protocol=info",
        "quinn_proto::connection=info",
        "refinery_core::traits::divergent=off",
        "lemoncraft_server::persistence::character=info",
        "lemoncraft_server::settings=info",
        "lemoncraft_query_server=info",
